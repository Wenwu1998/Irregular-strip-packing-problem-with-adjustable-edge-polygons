...
class QVNS:
    ...
    def _ql_select_action(self):
        if random.random() < max(self.ql_epsilon,0.05):
            action = random.randint(1, self.ql_num_actions)
        else:
            action = np.argmax(self.ql_Q[self.ql_state]) + 1
        self.ql_last_action = action -1 
        return action 


    def _ql_update(self, action, old_local_best, new_value):
        if new_value < self.bestvalue:
            new_state1 = 0
            self.ql_CF[action-1] = 0
        elif  new_value < old_local_best:
            new_state1 = 1
            self.ql_CF[action-1] = 0
        else:
            self.ql_CF[action-1] += 1
            new_state1= 2                
        if new_value < 0.8 * self.Inival:
            new_state2 = 0
        elif new_value < 0.92 * self.Inival:
            new_state2 = 1
        else:
            new_state2 = 2       
        new_state = new_state1 * 3 + new_state2   
        self.eta = [1,10,1] 
        term1 = self.eta[0] * (1 if new_value < self.bestvalue else 0)     
        term2 = self.eta[1]  * max(old_local_best - new_value, 0) / old_local_best
        term3 = self.eta[2]  * self.ql_CF[self.ql_last_action] / self.number_evaluations
        reward = term1 + term2 - term3
        old_q = self.ql_Q[self.ql_state, action-1]
        max_future_q = np.max(self.ql_Q[new_state])
        self.ql_Q[self.ql_state, action-1] = old_q + self.ql_alpha * (reward + self.ql_gamma * max_future_q - old_q)
        self.ql_epsilon *= self.ql_beta
        self.ql_state = new_state
    ...
    def generate_initial_solution(self):
       ...
       for idx in sorted_indices:
           if not packing:                
               ...
               last_group = packing[-1]
               if len(last_group) < 3 and idx in self.similar_list[last_group[-1]]:
    ...
    def run(self, ):
        ...
        for iteration in range(10000):
            ...
            while k <= self.k_max:
                ...
                self._ql_update(action, v_1, v_2)
                if v_2 < v_1:
